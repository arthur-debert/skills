...
description: >-
  ...
  when every project gets three mandatory tiers regardless of size, when a doc's
  first paragraph is about the document, when a component doc re-litigates
...
This skill governs the document level: which documents exist, what each one
owns, and what order a reader meets facts in. The sentence level — verdicts
where descriptions belong, abstract subjects, coined terms — is the `writing`
skill. A doc set can be perfectly structured and still unreadable sentence by
sentence, and vice versa; run both.
...
- **What.** The architecture: the parts, what each part is responsible for, and
  how they communicate. The reader is building a mental model before touching
  code.
...
Playing a role means answering its question, and each question has parts that
are easy to skip. The _what_ role includes how the parts relate to each other —
a document that exhaustively lists what each part owns and never says how they
...
- **A few kloc** (a CLI tool, a shell script collection): one README plays all
  ...
  learn one thing.
- **Tens to low hundreds of kloc** — the common case: a README that owns _why_
  and _what_, plus one doc per subsystem that has earned it. A subsystem earns a
  doc when its internals outgrow a section, not because the taxonomy has a slot
  for it.
...
summary and a link.** Split by pushing detail down, never by cloning a tier. The
inverse rule holds too: a document that would be under a screenful is a section
of its parent, not a file.
...
The detail was doing no work at this altitude; it becomes a child doc only when
...
- The same rule points up: the README does not carry schemas, and the
  ...
  deeper doc, the cross-reference _is_ the correct sentence — see the ownership
  test in `writing`.
...
- A glossary entry is a definition or it is deleted. A vocabulary section that
  lists bare words — "stage, store, key, transport" — defines nothing; it
  asserts ownership of fourteen terms while leaving all fourteen undefined.
...
still owes the reader who does own the refused thing, or what the reader gets
instead. "Update resolves; fetch never does" passes — it prices a plausible
assumption and pairs the negation with the positive half. "Declared nowhere"
alone does not.
...
as repetition, not residue. And a sentence arguing why the document breaks one
of these rules ("none of them names who is on the other side, because…") is
itself residue: the rationale documents the authoring policy, and does not waive
the rule.
...
The same applies to deliberation residue — options considered in the order
considered — and to session sediment (dates, provenance, verification
narration): see `writing` and `slop-clean`.
...
(you don't write one spec for a spreadsheet), never into a tier of child docs.

**Non-goals** follow the task-residue rules above, including the ration: name
the assumptions a reader would actually make, and stop.
...
5. **Split on overflow only.** Past ~200 lines, push the deepest section down
   into a child doc with a summary and link left behind. Never open an empty
   tier.
6. **Sweep for residue.** Every negation must price a real alternative; no term
...
- `writing` — the sentence level: descriptions not verdicts, actors as subjects,
  coined terms, ownership and self-consistency across a doc set. Always run
